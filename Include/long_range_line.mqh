// 水平線を作成する（長期時間軸）
bool LongLineCreate(string state)
{
  ...
  if (state == "high") {
    name = "HighLongLine";
    price = highOfLongRange;
  } else {
    name = "LowLongLine";
    price = lowOfLongRange;
  }
  color clr = C'253,150,76'; // 線の色
...
// 水平線を移動する（長期時間軸）
bool LongLineMove(string state)
{
...
// 水平線を削除する（使用していない）（長期時間軸）
bool LongLineDelete(string state)
{
  long chart_ID = 0;   // チャート識別子
  string name;
